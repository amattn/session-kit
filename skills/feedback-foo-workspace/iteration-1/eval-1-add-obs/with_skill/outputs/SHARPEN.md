...
### SPI_4: Agent skips running tests after making changes [friction]

User-reported. The agent makes code changes but does not run the test suite
afterward. This risks undetected regressions and forces the user to manually
remind the agent every time. Needs either a CLAUDE.md directive or a discipline
if the pattern persists after a simple rule is added.
